...
const RemoveCategoryButton: FC<RemoveCategoryButtonProps> = ({
	categoryId,
}) => {
	...
	const removedCategory = useSelector((state: RootStore) =>
		categoryListSelector.selectById(state, categoryId),
	);
	...
	const removeCategory = async (e: FormEvent<HTMLFormElement>) => {
		...
		dispatch(
			categoryActions.updateCategory({
				...
				category: { ...removedCategory, isHidden: true },
			}),
		);
		...
		try {
			...
			dispatch(
				categoryActions.updateCategory({
					updatedId: categoryId,
					category: { ...removedCategory, isHidden: false },
				}),
			);
		}
	};
		...
		<>
			...
			{isOpenRemoveModal && (
				<UiModal onHideCallback={() => setOpenRemoveModal(false)}>
					<form
						className={styles.removeCategoryForm}
						onSubmit={removeCategory}
						onReset={() => setOpenRemoveModal(false)}
					>
						<UiText>
						...
						<UiButtonsGroup>
							...
							<UiButton type="submit">Удалить</UiButton>
						</UiButtonsGroup>
					</form>
				</UiModal>
			)}

...
const ExpensesList: FC<ExpensesListProps> = ({ expensesList, expenseControlPanel, lastListItem = null }) => {
    ...
    const getCategoryLabels = (categoryId: string) => {
        ...
        return <CategoryLabel key={categoryId} color={categoryDict[categoryId].color} outline={true}> { categoryDict[categoryId]?.name ?? 'Нет имени' } </CategoryLabel>;
    }
    ...
    const getExpenseCard = (expenses: Expenses) => {
        ...
        return <ExpensesCard 
                    ...
                    labelsComponent={ expenses.categoryId ? getCategoryLabels(expenses.categoryId) : null } 
                />
    }

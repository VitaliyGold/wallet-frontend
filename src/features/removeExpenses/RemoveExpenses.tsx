...
const RemoveExpenses: FC<RemoveExpensesProps> = ({ onCancelCallback, onSuccessCallback, currentExpense }) => {

    const dispatch = useAppDispatch();

    const removeQuery = async () => {
        const { payload } = dispatch(expensesActions.patchExpense({ ...currentExpense, isHidden: true }));
        onSuccessCallback();
        try {
    ...
    return (
        <div className={styles.removeExpenses}>
            <p>Вы действительно хотите удалить трату {currentExpense.expensesName}?</p>
            <UiButtonsGroup>
                <UiButton viewType="transparent" outline onClick={onCancelCallback}>
                    Отмена
                </UiButton>
                <UiButton onClick={removeQuery}>
                    Удалить
                </UiButton>
            </UiButtonsGroup>
        </div>

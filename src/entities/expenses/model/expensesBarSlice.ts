...
const ExpensesBarSlice = createSlice({
	...
	extraReducers(builder) {
		builder.addCase(getExpensesGroupByCategoryThunk.pending, (state) => {
			state.loading = true;
			state.error = false;
		}),
			builder.addCase(
				getExpensesGroupByCategoryThunk.rejected,
				(state) => {
					state.loading = false;
					state.error = true;
				},
			...
			builder.addCase(
				getExpensesGroupByCategoryThunk.fulfilled,
				(state, action) => {
					state.loading = false;
...
export const { actions: expensesBarActions, reducer: expensesBarReducer } =
	ExpensesBarSlice;

export { expensesBarAdapter };

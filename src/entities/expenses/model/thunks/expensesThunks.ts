...
const getExpensesListThunk = createAsyncThunk('expenses/list', async ({ limit, offset, name, startDate, endDate, categoryIds, direction }: GetExpenseRequestParams ): Promise<PaginationResponse<Expenses[]>> => {
    const { data, total } = await getExpensesListApi(name, limit, offset, startDate, endDate, categoryIds, direction);
    return {

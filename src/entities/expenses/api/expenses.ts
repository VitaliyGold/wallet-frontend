...
const getExpensesListApi = (
	...
	endDate: number,
	category_ids: string[],
): Promise<PaginationResponse<ExpenseApiWithCategoryAndTags[]>> => {
	const query = {
		name: name,
		limit: String(limit),
...
const saveNewExpensesApi = (
	expenses: SetExpenseApi[],
): Promise<ExpenseApiWithCategoryAndTags[]> => {
	return fetcher.post("expenses", expenses);
};

const removeExpensesApi = (
	expenseId: string,
): Promise<RemoveExpenseApiResponse> => {
	return fetcher.delete("expenses", { expenses_id: expenseId });
};

const editExpensesApi = (
	updatedExpense: SetExpenseApi,
): Promise<ExpenseApiWithCategoryAndTags> => {

...
const AddNewExpenseCard: FC<AddNewExpenseCardProps> = ({ addNewExpense }) => {
	const [isOpenModalCreateExpense, setModalCreateExpenseState] =
		useState(false);
	...
	return (
		<>
			<div
				className={styles.addNewExpenseCard}
				title="Добавить трату"
				onClick={() => setModalCreateExpenseState(true)}
			>
				<Icon iconType="plus" size={52} />
			</div>
			{isOpenModalCreateExpense && (
				<UiModal
					onHideCallback={() => setModalCreateExpenseState(false)}
				>
					<ExpensesForm
						closeCallback={() => setModalCreateExpenseState(false)}
						saveCallback={(expense) => onAddNewExpense(expense)}
					/>
				</UiModal>
			)}
		</>

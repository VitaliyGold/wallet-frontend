...
const ExpensesChart = () => {
	...
	const getData = () =>
		dispatch(
			getExpensesGroupByCategoryThunk({
				...
				endDate: new Date("2024-09-29").getTime(),
			}),
		);
	...
	const tooltipContent = (tooltipProps: TooltipProps<number, string>) => {
		...
		const hoverCategoryId =
			tooltipProps.payload && tooltipProps.payload[0]?.name;

		const hoverCategoryAmount =
			(tooltipProps.payload && tooltipProps.payload[0]?.value) ?? 0;

		return hoverCategoryId ? (
			<ChartTooltip
				categoryName={getCategoryName(hoverCategoryId)}
				amount={hoverCategoryAmount}
			/>
		) : null;
	};
	...
	return (
		<div className={styles.chartContainer}>
			<ResponsiveContainer>
				<PieChart>
					<Pie
						data={expensesGroupData}
						dataKey="totalAmount"
						nameKey="categoryId"
					>
						{expensesGroupData.map((expense) => (
							<Cell
								key={expense.categoryId}
								name={getCategoryName(expense.categoryId)}
								fill={getColorByCategoryId(expense.categoryId)}
							/>
						))}
					</Pie>
